...
class liveStream {
    ...
    createClient(options,success){
        ...
        liveStream.client.on('stream-subscribed', function (evt) {
            ...
            console.log("%c触发stream-subscribed","font-size: 20px;color: gold;")
            ...
            _this.subscribedStreamCallBack(originStream);
        });
    ...
    subscribedStream(success=()=>{}){
        this.subscribedStreamCallBack = success;
    }
    ...
    */
    ...
    //禁摄像头后，cdn合图不展现连麦画面
    disVideoList(disvideoList){
        console.log("执行1================");
        if(config.role == "Broadcaster"){
            this.disvideoList = disvideoList;
    ...
    cdnSetTransCode(){
        let j = 0;
        let position = 0;
        this.cdnUsersArray.forEach((every,z)=>{
            var indexs;
            // 主播画面为 1280 * 720 不改变
            console.log('执行3========',this.disvideoList);
            if(this.disvideoList.length > 0){
                this.disvideoList.forEach((item,index)=>{
                    if(every["uid"] == item){
                        // 禁用摄像头
                        indexs = z;
                        every.width = 1;
            ...
            if(indexs != z && z != 0){
                ...
                j++;
                every.width = this.cdnUsersWidth;
                every.height = Math.ceil((this.cdnUsersWidth/4)*3);
                every.x = position + (4 * (j == 1 ? 0 : (j-1)));
                position = 0;
            }
            console.log("禁止摄像头后重新绘图=====",this.cdnUsersArray);
        })
        var LiveTranscoding = {
          width: 1280,
          height: 720,
    ...
    closeVedioHandle(flag,stream){
        if(!this.screenFlag){
            ...
            // flag?conStream.muteVideo():conStream.unmuteVideo();  // 2.5.1 方法
            flag?conStream.disableVideo():conStream.enableVideo();
        }
    ...
    closeAudioHandle(flag,stream){
        let conStream = stream?stream:liveStream.hostStream;
        // flag?conStream.muteAudio():conStream.unmuteAudio();   // 2.5.1 方法
        flag?conStream.disableAudio():conStream.enableAudio();
